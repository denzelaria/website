...
const SubjectSegment = ({title, icon, notes}:{title:string, icon:string, notes:string[]}) => {
    ...
    return (
        <div className="hover:cursor-pointer relative bg-white dark:bg-zinc-900 md:w-[40vw] not-md:w-[80vw] rounded p-5 border border-zinc-700" onClick={() => setOpen(!open)}>
            <div className="flex flex-row rounded z-20 relative justify-between">
                <div className="flex flex-row gap-2 overflow-hidden">
                    <p className={`select-none ${ open ? 'rotate-0' : 'rotate-90' } duration-300`}>&gt;</p>
                    <h3 className="select-none text-xl wrap-break-word">{title}</h3>
                </div>
